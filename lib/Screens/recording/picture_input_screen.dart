      ...
      for (int i = 0; i < _selectedImages.length; i++) {
        try {
          ...
          if (_selectedImages.length > 1) {
            imageDescription +=
                ' (Image ${i + 1} of ${_selectedImages.length})';
          }
          ...
          String categoryIdToUse =
              (_categoryId != null && UuidService.isValidUuid(_categoryId!))
                  ? _categoryId!
                  : 'default'; // Provide a default value instead of null

          final result = await ApiService.createAndUploadRecord(
            title:
                '${_titleController.text.trim()}${_selectedImages.length > 1 ? ' - Image ${i + 1}' : ''}',
          ...
          if (result['success'] == true) {
            successCount++;
            print(
                'Successfully uploaded image ${i + 1}: ${result['message'] ?? 'Success'}');
          } else {
            failureCount++;
            print(
                'Upload failed for image ${i + 1}: ${result['message'] ?? 'Unknown error'}');
          }
        } catch (e) {
          failureCount++;
          ...
          if (e.toString().contains('User not authenticated') ||
            ...
            _showSnackBar(
                'Authentication failed. Please login again.', Colors.red);
  ...
  Future<void> _saveDraft() async {
    ...
    try {
      ...
      if (recordResponse['success'] == true) {
        ...
        _showSnackBar(
            'Failed to save draft: ${recordResponse['message'] ?? 'Unknown error'}',
            Colors.red);
  ...
  Future<void> _refreshCategoryData() async {
    try {
      if (_effectiveCategory != null) {
        final newCategoryId =
            await UuidService.getCategoryUuid(_effectiveCategory!);
        if (newCategoryId != null && newCategoryId != _categoryId) {
          setState(() {
            _categoryId = newCategoryId;
          });
          print(
              'Updated category ID: $_categoryId for category: $_effectiveCategory');
        }
      }
    } catch (e) {
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          TextField(
                            ...
                            decoration: const InputDecoration(
                              hintText:
                                  'Share your thoughts, story, or context...',
                              border: InputBorder.none,
                ...
                children: [
                  ...
                  Expanded(
                    child: OutlinedButton(
                      onPressed:
                          _isProcessing || _isUploading ? null : _saveDraft,
                      style: OutlinedButton.styleFrom(
                        foregroundColor: kPrimaryColor,
