...
class _VideoInputScreenState extends State<VideoInputScreen> {
  ...
  // NEW: URL encoding method for Telugu characters
  String _encodeTitle(String title) {
    try {
      // URL encode the title to handle Telugu characters
      String encoded = Uri.encodeComponent(title);
      print('DEBUG: Original title: "$title"');
      print('DEBUG: Encoded title: "$encoded"');
      return encoded;
    } catch (e) {
      print('ERROR: Failed to encode title: $e');
      // Fallback to original title if encoding fails
      return title;
    }
  }

  // NEW: Decode title for display purposes
  String _decodeTitle(String encodedTitle) {
    try {
      return Uri.decodeComponent(encodedTitle);
    } catch (e) {
      print('ERROR: Failed to decode title: $e');
      return encodedTitle;
    }
  }

  // NEW: Validate if title contains Telugu characters
  bool _containsTeluguCharacters(String text) {
    // Telugu Unicode range: U+0C00 to U+0C7F
    final teluguRegex = RegExp(r'[\u0C00-\u0C7F]');
    return teluguRegex.hasMatch(text);
  }
  ...
  // MODIFIED: Process videos with URL encoding for Telugu characters
  Future<void> _processVideos() async {
    ...
    try {
      ...
      String originalTitle = _titleController.text.trim();
      String originalDescription = _descriptionController.text.trim();

      // NEW: Check if title contains Telugu characters and encode if necessary
      String titleToSend = originalTitle;
      bool titleContainsTelugu = _containsTeluguCharacters(originalTitle);
      
      if (titleContainsTelugu) {
        titleToSend = _encodeTitle(originalTitle);
        print('DEBUG: Telugu characters detected in title, encoding applied');
        _showSnackBar('Telugu characters detected - applying encoding', Colors.blue);
      }

      // NEW: Also encode description if it contains Telugu characters
      String descriptionToSend = originalDescription;
      bool descriptionContainsTelugu = _containsTeluguCharacters(originalDescription);
      
      if (descriptionContainsTelugu) {
        descriptionToSend = _encodeTitle(originalDescription);
        print('DEBUG: Telugu characters detected in description, encoding applied');
      }

      // Create the record using the createRecord method with encoded parameters
      final recordResult = await ApiService.createRecord(
        title: titleToSend, // Use encoded title
        description: descriptionToSend, // Use encoded description
      ...
      if (recordResult['success']) {
        ...
        if (extractedId == null) {
          recordData.forEach((key, value) {
            if ((key.toLowerCase().contains('id') ||
              ...
              print('Found potential ID in field: $key = $extractedId'); // Debug log
            }
          });
        }
        ...
        if (_createdRecordId != null && _createdRecordId!.isNotEmpty) {
          // NEW: Show success message with encoding info
          if (titleContainsTelugu || descriptionContainsTelugu) {
            _showSnackBar('Record created with Telugu text encoding', Colors.green);
          }
          _showSubmitDialog();
        } else {
        ...
        // NEW: Enhanced error handling for encoding-related issues
        String errorMessage = recordResult['message'] ?? recordResult['error'] ?? 'Unknown error';
        
        if (errorMessage.contains('unsupported metadata value') || 
            errorMessage.contains('US-ASCII') ||
            errorMessage.contains('encoding')) {
          _showSnackBar(
              'Character encoding error detected. Please try again or contact support.',
              Colors.red);
          print('ENCODING ERROR: $errorMessage');
        } else {
          _showSnackBar('Error creating record: $errorMessage', Colors.red);
        }
        
        print('Create record failed: $recordResult'); // Debug log
      }
    } catch (e) {
      // NEW: Enhanced error handling for encoding exceptions
      String errorMessage = e.toString();
      if (errorMessage.contains('encoding') || errorMessage.contains('ASCII')) {
        _showSnackBar('Text encoding error: $e', Colors.red);
  ...
  // MODIFIED: Upload videos with enhanced encoding support
  Future<void> _uploadVideos() async {
    ...
    try {
      ...
      for (int i = 0; i < _selectedVideos.length; i++) {
        ...
        try {
          // NEW: Prepare encoded description for upload
          String originalDescription = _descriptionController.text.trim();
          String uploadDescription = '$originalDescription - Video ${i + 1}';
          
          if (_containsTeluguCharacters(uploadDescription)) {
            uploadDescription = _encodeTitle(uploadDescription);
            print('DEBUG: Encoding description for video ${i + 1}');
          }
          ...
          final uploadResult = await ApiService.uploadRecord(
            ...
            file: file,
            title: _titleController.text.trim(), // Keep original title for display
            categoryId: _getCategoryId(), // Use the helper method to get category ID
            userId: _userId!, // Use the authenticated user ID
            mediaType: 'video',
            description: uploadDescription, // Use potentially encoded description
          );
                              ...
                              Container(
                                ...
                                child: TextField(
                                  ...
                                  decoration: InputDecoration(
                                    labelText: 'Title *',
                                    hintText:
                                        'Give your video an engaging title (Telugu supported)',
                                    ...
                                    prefixIcon: const Icon(Icons.edit,
                                        color: kPrimaryColor),
                                    suffixIcon: _containsTeluguCharacters(_titleController.text)
                              ...
                              Container(
                                ...
                                child: TextField(
                                  ...
                                  decoration: InputDecoration(
                                    ...
                                    hintText:
                                        'Tell viewers what your video is about... (Telugu supported)',
                                    border: OutlineInputBorder(
                                      borderRadius: BorderRadius.circular(12),
                                      borderSide: BorderSide.none,
                                    ),
                                    ...
                                    suffixIcon: _containsTeluguCharacters(_descriptionController.text)
                                        ? const Padding(
                                            padding: EdgeInsets.only(bottom: 60),
                                            child: Icon(Icons.translate, color: Colors.blue, size: 20),
                                          )
                                        : null,
                                  ),
                                ),
                        ...
                        Container(
                          ...
                          child: Column(
                            ...
                            children: [
                              ...
                              _buildTipItem('🔤',
                                  'Telugu text is automatically encoded for upload'),
                            ],
                          ),
                        ),
                      ],
