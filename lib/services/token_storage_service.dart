...
class TokenStorageService {
  ...
  // Get current user data from API using stored token
static Future<Map<String, dynamic>?> getCurrentUserFromApi() async {
  try {
    ...
    final url = Uri.parse('https://backend2.swecha.org/api/v1/auth/me');
    final headers = {
    ...
    print('Get current user response: ${response.statusCode}, ${response.body}');
...
static Future<bool> storeUserProfile({
  ...
  try {
    ...
    if (dateOfBirth != null) await prefs.setString('profile_date_of_birth', dateOfBirth);
    if (place != null) await prefs.setString('profile_place', place);
    if (userId != null) await prefs.setString('profile_user_id', userId);
...
static Future<Map<String, String?>> getStoredUserProfile() async {
  try {
    final prefs = await SharedPreferences.getInstance();
    return {
      ...
      'place': prefs.getString('profile_place'),
      'userId': prefs.getString('profile_user_id'),
    };
  } catch (e) {
...
static Future<bool> clearUserProfile() async {
  try {
    ...
    await prefs.remove('profile_place');
...
Future<void> _debugAllStorageKeys() async {
  try {
    final prefs = await SharedPreferences.getInstance();
    ...
    print('DEBUG: Error reading all SharedPreferences: $e');
  }
}
